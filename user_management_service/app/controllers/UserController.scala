...
class UserController @Inject()(cc: UserControllerComponents)(
  ...
  def login: Action[CredentialsValidator] = silhouette.UnsecuredAction.async(
      parse.json[CredentialsValidator]) { implicit request =>
    userResourceHandler.find(request.body.username) semiflatMap { userResource =>
      val credentials = Credentials(userResource.id.get.toString, request.body.password)
      credentialsProvider.authenticate(credentials)
        .flatMap { loginInfo =>
          ...
          case _: InvalidPasswordException => Forbidden("Invalid credentials")
