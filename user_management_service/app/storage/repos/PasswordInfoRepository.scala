...
class PasswordInfoRepository @Inject() (
  ...
  protected def addAction(
      loginInfo: LoginInfo,
      authInfo: PasswordInfo
    ) = {
    for {
      loginInfoId <- (loginInfos returning loginInfos.map(_.id)) +=
        DBLoginInfo(None, loginInfo.providerID, loginInfo.providerKey.toLong)
      _ <- passwordInfoQuery +=
        DBPasswordInfo(
          loginInfoId,
          authInfo.hasher,
  ...
  override def save(
      loginInfo: LoginInfo,
      authInfo: PasswordInfo
    ): Future[PasswordInfo] = db.run {
    loginInfoQuery(loginInfo)
      .joinLeft(passwordInfoQuery)
